...
def main_loop():
    ...
    grid = Grid(CELL_SIZE, '34life', aging=True)
    capture = Capture(grid)

    chances = list([1 for dead in range(int(1 / DEAD_RATIO - 1))])
    chances.append(0)
    seedline_area = grid.num_columns/4
    seedline_cols = set([random.randrange(seedline_area, seedline_area+50) for column in range(random.randrange(10, 20))])
